...
const AdminDashboard: React.FC = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const token = localStorage.getItem('admin_token');
      const response = await axios.get<WorkerSubmission[]>(`${API}/submissions`, {
  ...
  const fetchJobTypes = async () => {
    try {
      const response = await axios.get<{ job_types: string[] }>(`${API}/job-types`);
  ...
  const handleCreateArtisan = async (e: React.FormEvent) => {
    ...
    if (!artisanForm.name || !artisanForm.phone || !artisanForm.location || !artisanForm.job_type) {
    ...
    try {
      ...
      if (editingArtisan) {
        await axios.put(`${API}/artisans/${editingArtisan.id}`, artisanForm, {
          headers: { Authorization: `Bearer ${token}` }
        });
        toast.success('Artisan updated successfully');
      } else {
        await axios.post(`${API}/artisans`, artisanForm, {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      setArtisanForm({ name: '', phone: '', location: '', job_type: '', photo_url: '' });
      fetchArtisans();
    } catch (error) {
  ...
  const handleApproveSubmission = async (id: string) => {
    try {
      const token = localStorage.getItem('admin_token');
      await axios.put(`${API}/submissions/${id}/approve`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-32 pb-20">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="mb-8">
          <h1 className="text-5xl font-black tracking-tight text-slate-900 mb-2" data-testid="dashboard-title">
            Admin Dashboard
          </h1>
          <p className="text-lg text-slate-600">Manage artisans and worker submissions</p>
        </div>
        ...
        <Tabs defaultValue="artisans" className="w-full">
          ...
          <TabsContent value="artisans">
            ...
            <div className="bg-white border border-slate-200 overflow-x-auto" data-testid="artisans-table">
              <table className="w-full">
                <thead className="bg-slate-100 border-b border-slate-200">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-bold text-slate-900">Name</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-slate-900">Job Type</th>
                    ...
                    <th className="px-6 py-4 text-left text-sm font-bold text-slate-900">Rating</th>
                    <th className="px-6 py-4 text-right text-sm font-bold text-slate-900">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {artisans.map((artisan) => (
                    <tr key={artisan.id} className="border-b border-slate-100" data-testid={`artisan-row-${artisan.id}`}>
                      <td className="px-6 py-4 text-slate-900 font-medium">{artisan.name}</td>
                      <td className="px-6 py-4 text-slate-600">{artisan.job_type}</td>
                      <td className="px-6 py-4 text-slate-600">{artisan.location}</td>
                      <td className="px-6 py-4 text-slate-600">{artisan.phone}</td>
                      <td className="px-6 py-4 text-slate-600">
                        {artisan.average_rating?.toFixed(1) || 0} ({artisan.total_reviews || 0})
                      </td>
              ...
              {artisans.length === 0 && (
                <div className="p-12 text-center text-slate-600" data-testid="no-artisans">
          ...
          <TabsContent value="submissions">
            <div className="bg-white border border-slate-200 overflow-x-auto" data-testid="submissions-table">
              <table className="w-full">
                ...
                <tbody>
                  {submissions.map((submission) => (
                    <tr key={submission.id} className="border-b border-slate-100" data-testid={`submission-row-${submission.id}`}>
                      <td className="px-6 py-4 text-slate-900 font-medium">{submission.name}</td>
                      <td className="px-6 py-4 text-slate-600">{submission.job_type}</td>
                      <td className="px-6 py-4 text-slate-600">{submission.location}</td>
                      <td className="px-6 py-4 text-slate-600">{submission.phone}</td>
                      <td className="px-6 py-4">
                        <span className={`inline-block px-3 py-1 text-xs font-bold tracking-wide uppercase ${
                          submission.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                        }`}>
                          {submission.status}
                        </span>
                      </td>
              ...
              {submissions.length === 0 && (
                <div className="p-12 text-center text-slate-600" data-testid="no-submissions">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setArtisanForm({ ...artisanForm, job_type: e.target.value })}
                  className="w-full h-12 bg-white border-2 border-slate-200 focus:border-primary focus:ring-0 px-4"
